...
# Custom Claude Code statusline - Created: 2025-11-06T05:20:55.562Z
...
if [ "$HAS_JQ" -eq 1 ]; then
  ...
  # Extract model id (used for context window detection, e.g. [1m] variants)
  model_id=$(echo "$input" | grep -o '"model"[[:space:]]*:[[:space:]]*{[^}]*"id"[[:space:]]*:[[:space:]]*"[^"]*"' | sed 's/.*"id"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/')
...
# Determine max context based on model id/name
# - 모델 id에 "1m" 변형이 있으면 1M context로 인식
# - 그 외 Claude 4.x / Sonnet 3.5+ 는 200K
# - 원조 Claude 3 Haiku는 100K
get_max_context() {
  local model_name="$1"
  local model_id="$2"
  case "$model_id" in
  ...
  case "$model_name" in
    ...
    *"Claude 3 Haiku"*|*"claude 3 haiku"*)
      ...
      ;;
    *)
      echo "200000"  # Default to 200K (Opus/Sonnet/Haiku 4.x 등)
      ;;
  esac
}

if [ -n "$session_id" ] && [ "$HAS_JQ" -eq 1 ]; then
  MAX_CONTEXT=$(get_max_context "$model_name" "$model_id")
